...
**Contact form is front-end only.** `#contact-form` in the Contact section does client-side validation (`script.js`) but has no `action`/network submission wired up — there's a comment in `index.html` right above the `<form>` marking where to add a real integration (e.g. Formspree, Netlify Forms) when one is chosen.
